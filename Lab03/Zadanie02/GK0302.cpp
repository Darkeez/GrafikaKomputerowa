...
/*
Program 2. Opracować program przedstawiający trzy różne sposoby utworzenia mipmap dla jednowymiarowej tekstury.
Pierwsza technika korzysta z rysunków przygotowanych w programie graficznym o kolejnych rozmiarach: 512×1, 256×1, 128×1, 64×1, 32×1, 16×1, 8×1, 4×1, 2×1 i 1×1. 
Każda z tych tekstur (mipmap) jest kolejno definiowana przy uzyciu funkcji glTexImage1D.
Dla uproszczenia tekstury zawarto w tekscie zródłowym programu w postaci tablic. Oczywiście w przypadku tekstur dwu czy trójwymiarowych takiego rozwiązania nie praktykuje się.
Tekstury są najczęściej przechowywane w plikach graficznych lub plikach o formacie przygotowanym na potrzeby konkretnej aplikacji.
Przy drugiej teksturze do generowania mipmap użyto funkcji gluBuild1DMipmaps. 
Natomiast w trzecim przypadku wykorzystano mechanizm automatycznego generowania mipmap (program sprawdza numer wersji biblioteki OpenGL).
Ponadto program umożliwia wybór jednego z czterech sposobów filtrowania mipmap dla filtra pomniejszającego
...
void Init()
{
	//Funkcja ta ustala kolor stosowany do czyszczenia zawartości okna.
	glClearColor(0.7f, 0.2f, 0.3f, 1.0f);
	// włączenie teksturowania jednowymiarowego
	glEnable(GL_TEXTURE_1D);
	// utworzenie identyfikatora tekstury
	glGenTextures(3, textures);
	// włączenie teksturowania jednowymiarowego, dowiazanie stanu
	...
	//metoda zawijania tekstury w kierunku współrzędnej S
	...
	//Wygenerowanie  tekstury przez bibiloteke
...
void Display()
{
	// Wyczyszczenie okna aktualnym kolorem czyszczącym i bufora głębi
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	// włączenie teksturowania jednowymiarowego
	glBindTexture(GL_TEXTURE_1D, textures[0]);
	// Rozpoczęcie rysowania kwadratu
	glBegin(GL_QUADS);
	glTexCoord1f(0.0f);
	...
	// Wysłanie poleceń do wykonania i zamiana buforów
	glutSwapBuffers();
}
...
int main(int argc, char** argv)
{
	...
	//zdefiniowana wcześniej funkcja będzie funkcja zwrotna {callback function) wyświetlająca obraz na ekranie.
	glutDisplayFunc(Display);

	/*
	Ta funkcja uruchamia szkielet biblioteki GLUT. Po zdefiniowaniu funkcji zwrotnej obsługującej rysowanie na ekranie i
	innych funkcji uruchamiany jest mechanizm biblioteki. Powrót z funkcji
	glutMainLoop następuje dopiero przy zakończeniu pracy programu, dlatego wystarczy wywołać ją tylko raz. Funkcja ta
	przetwarza wszystkie komunikaty systemu operacyjnego, naciśnięcia klawiszy i inne, do czasu zakończenia pracy
	programu.
	*/
	glutMainLoop();
